...
| Multi-agent workflows | 21 specialized agents for proofreading, layout, pedagogy, code review (R/Python/Stata), fact-checking, AI-voice auditing, simulation + R-package review |
...
You: "Analyze this dataset and produce publication-ready output"
     ↓
Claude invokes /data-analysis-r, which internally:
...
Claude Code ships with built-in skills beyond this template's 58: `/batch` orchestrates parallel refactoring across your codebase (using git worktrees for isolation), `/simplify` runs 3-agent code review and applies fixes, and `/debug` helps troubleshoot sessions. These complement the academic skills above.
:::
...
Not all agents need the same model. Each agent file has a `model:` field in its YAML frontmatter. All 21 agents are **pinned** to an explicit model + effort tier (no longer `inherit`, since v2.0), following the 70/20/10 routing pattern below. You can re-tune any agent to optimize cost:
...
These skills support the research workflow beyond slide development:
...
| `/data-analysis-python [data]` · `/data-analysis-stata` · `/data-analysis-r [data]` (R) | End-to-end analysis pipeline in your project's language(s): explore, clean (with validation), estimate, produce publication-ready output | Empirical analysis phase |

These skills produce structured reports saved to `quality_reports/`. The three analysis skills share one pipeline shape in three languages — pick per project via the language-roles table in `CLAUDE.md` (e.g. prep = Python, estimation = Stata, cross-check = R). Each generates numbered scripts (`scripts/python/`, `scripts/stata/`, `scripts/R/`), runs its language's reviewer agent automatically, and invokes `/cross-check` after estimation to re-verify headline results in a second language.
...
```
/research-ideation ─────┐
                        ├──→ /lit-review ──→ /data-analysis-r ──→ /review-paper
/interview-me ──────────┘         ↑               ↑
                                  │               │
                          (can run in parallel)   │
                                                  │
...
**Enter mid-pipeline.** You do not have to start from ideation. If you already have data, start with `/data-analysis-r` and work backwards to the research question. If you already have a draft, start with `/review-paper`. The skills are modular --- use what you need, skip what you don't.
...
**Why it matters for this template:** [`/data-analysis-stata`](#sec-appendix) (v1.9.0) depends on this MCP server, and so does the Stata leg of [`/cross-check`](#sec-appendix). The analysis pipeline comes in three parallel languages — [`/data-analysis-python`](#sec-appendix), `/data-analysis-stata`, and [`/data-analysis-r`](#sec-appendix) (R) — same pipeline shape, picked per project via the language-roles table in `CLAUDE.md`. AEA submissions where the replication package is in Stata are the canonical use case for this server.
...
The guide includes 58 [skills](#skills---reusable-slash-commands) for common academic tasks. But if you have repetitive workflows specific to your domain, you can create your own.
...
| Python Reviewer | `.claude/agents/python-reviewer.md` | Python code quality, reproducibility, pandas/estimation idioms, numerical discipline (v2.0) |
| Stata Reviewer | `.claude/agents/stata-reviewer.md` | Stata reproducibility scaffolding, clustering/inference discipline, data-handling traps, esttab/AEA compliance (v2.0) |
...
| Plan Auditor | `.claude/agents/plan-auditor.md` | Fresh-context fidelity audit of analysis plans against the user's verbatim interview log (v2.1) |
...
| `/review-python` | `.claude/skills/review-python/` | Run Python code review agent (v2.0) |
| `/review-stata` | `.claude/skills/review-stata/` | Run Stata code review agent (v2.0) |
...
| `/data-analysis-r` | `.claude/skills/data-analysis-r/` | End-to-end R analysis |
| `/data-analysis-python` | `.claude/skills/data-analysis-python/` | End-to-end Python analysis (pandas/statsmodels/linearmodels/pyfixest) with a mandatory data-prep validation battery (v2.0) |
| `/audit-reproducibility` | `.claude/skills/audit-reproducibility/` | Enforce replication tolerance thresholds on paper ↔ code |
| `/diagnose` | `.claude/skills/diagnose/` | Root-cause a wrong or failing empirical result: reproduce → minimise → hypothesise → instrument → fix (v2.0) |
...
| `/data-analysis-stata` | `.claude/skills/data-analysis-stata/` | End-to-end Stata pipeline scaffold + execution via the `stata-mcp` MCP server (the Stata member of the R/Python/Stata pipeline triad; v1.9.0) |
| `/cross-check` | `.claude/skills/cross-check/` | Independent re-implementation of a result in a second language (any Python ↔ Stata ↔ R pair) against `replication-protocol.md` tolerances; `--data` mode for cleaned datasets; auto-invoked after estimation (v2.0) |
| `/analysis-plan` | `.claude/skills/analysis-plan/` | Living analysis plan — spec table the pipelines execute by ID; `plan-auditor` fidelity gate (v2.1) |
| `/onboard-project` | `.claude/skills/onboard-project/` | Existing-project intake — dossier extraction + non-destructive migration into a standalone template repo (v2.1) |
...
| `/setup-machine` | `.claude/skills/setup-machine/` | Bootstrap a fresh Mac/Windows machine — OS-detected toolchain install, standalone uv Python env, R/Stata packages, stata-mcp, LF guardrail, then validate (v2.1) |
