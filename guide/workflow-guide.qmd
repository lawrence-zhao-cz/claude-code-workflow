...
| Multi-agent workflows | 20 specialized agents for proofreading, layout, pedagogy, code review (R/Python/Stata), fact-checking, AI-voice auditing, simulation + R-package review |
...
Claude Code ships with built-in skills beyond this template's 55: `/batch` orchestrates parallel refactoring across your codebase (using git worktrees for isolation), `/simplify` runs 3-agent code review and applies fixes, and `/debug` helps troubleshoot sessions. These complement the academic skills above.
:::
...
Not all agents need the same model. Each agent file has a `model:` field in its YAML frontmatter. All 20 agents are **pinned** to an explicit model + effort tier (no longer `inherit`, since v2.0), following the 70/20/10 routing pattern below. You can re-tune any agent to optimize cost:
...
The guide includes 55 [skills](#skills---reusable-slash-commands) for common academic tasks. But if you have repetitive workflows specific to your domain, you can create your own.
