...
- **`/onboard-project`** — existing-project intake: deep-scan + provenance-cited Project Dossier + non-destructive migration into a standalone template repo; pre-seeds `/interview-me` and `/analysis-plan` (dossier-aware); `plan-auditor` gains a dossier mode.
- **`/analysis-plan`** + the `plan-auditor` agent — the living, user-approved analysis plan (cleaning filters, specification table, output registry) that the pipelines execute by ID, the reviewers check code against, and `/cross-check` re-implements from; a fresh-context auditor verifies the plan against the user's verbatim words (the intent-error gate).
