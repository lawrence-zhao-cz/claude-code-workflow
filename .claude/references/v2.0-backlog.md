...
- ✅ **Capability skills** — none of `/syllabus`, `/teach-from-paper`, `/power-analysis`, `/replication-package`, `/respond-to-eval` shipped this cycle (these remain deferred — see below). However, **6 *new* skills** outside the original v1.9 capability list shipped: `/humanize`, `/prompt`, `/prompt-only`, `/compress-session`, `/promote-memory`, `/data-analysis-stata`. See [CHANGELOG.md v1.9.0](../../CHANGELOG.md) for full details.
...
- ✅ **Stata expansion** — `/data-analysis-stata` + `.claude/rules/stata-code-conventions.md` + `/audit-reproducibility` Stata coverage shipped (Pass 4). Was not originally on the v1.9 backlog but added per user request mid-cycle.
...
- **sample `.mcp.json`** — opt-in wiring for Gmail / Calendar + econ-data MCPs (FRED / World Bank / BLS) + Zotero. *(A populated `.mcp.json` for `stata-mcp` shipped with the Python/Stata customization — this item now covers only the remaining connectors.)*
